...
namespace Tests
{
  ...
  public class TestBase
  {
    ...
    public const int TEST_THREAD_COUNT = 4; // ideal 4 on mbp 2019 8c
    ...
    protected static ITerm list(params int[] terms) => TermList.FromEnumerable(Enumerable.Select(terms, t => (ITerm)constterm(t)));
    ...
    protected static Proj proj(IProgram source, (NameVar, NameVar)[] projections) => new Proj(source, new ProjectionMap(projections.Select(tu => new KeyValuePair<NameVar, NameVar>(tu.Item1, tu.Item2)).ToArray()));
    protected static LibraryProgram lt => MathLib.lt;
    protected static LibraryProgram leq => MathLib.leq;
    protected static LibraryProgram plus => MathLib.plus;
    protected static LibraryProgram min => MathLib.min;
    protected static LibraryProgram max => MathLib.max;
    #endregion
    ...
    protected ProgramEnvironment assertFirstResultFor(string domains, string atusStr, string expectedProgramString, string negativeExamplesStr)
    {
      return assertFirstResultFor(domains, atusStr, expectedProgramString, negativeExamplesStr, TEST_SEARCH_DEPTH, TEST_ALLOWED_UNBOUND_ARGS);
    }
    ...
    {
      return assertFirstResultFor(domains, atusStr, expectedProgramString, negativeExamples: "{}", searchDepth, TEST_ALLOWED_UNBOUND_ARGS);
    }

    protected ProgramEnvironment assertFirstResultFor(string domains, string atusStr, string expectedProgramString, int searchDepth, int allowedUnboundArguments = TEST_ALLOWED_UNBOUND_ARGS)
    {
      return assertFirstResultFor(domains, atusStr, expectedProgramString, negativeExamples: "{}", searchDepth, allowedUnboundArguments);
    }

    protected ProgramEnvironment assertFirstResultFor(string domains, string atusStr, string expectedProgramString, string negativeExamples, int searchDepth, int allowedUnboundArguments)
    {
      ProgramEnvironment preEnv = buildEnvironment(domains, atusStr, searchDepth, allowedUnboundArguments);
      var negExamples = Parser.ParseGroundRelation(negativeExamples, preEnv.Frees);
      SynthesisJob job = new SynthesisJob(preEnv, negExamples, new ThreadCount(TEST_THREAD_COUNT), SearchOptions.FindOnlyFirstProgram);
    ...
    protected void assertNoResultFor(string domains, string atusStr, int searchDepth = TEST_SEARCH_DEPTH, int allowedUnboundArguments = TEST_ALLOWED_UNBOUND_ARGS)
    {
      ...
      SynthesisJob job = new SynthesisJob(env, GroundRelation.Empty, new ThreadCount(TEST_THREAD_COUNT), SearchOptions.FindAllPrograms);
      var programs = job.FindPrograms();
